...
int main(int argc, const char **argv)
{
    if (argc >= 2)
    {
        ...
        if (argc == 3) // if AST flag is present
        {
            if (strcmp(argv[1], "-ast") == 0) // if -ast flag is present as first argument
            {
                filePathIndex = 2;
                isASTFlagged = true;
            }
            else if (strcmp(argv[2], "-ast") == 0) // if -ast flag is present as second argument
            {
                filePathIndex = 1;
                isASTFlagged = true;
            }
        ...
        else if (argc == 2) // if no flag is present
        {
            filePathIndex = 1;
        }
        else
        ...
        if (!input.is_open()) // Check if file exists
        {
        ...
        for (int i = 0; i < inputString.size(); i++)
            characterArray[i] = inputString[i];
